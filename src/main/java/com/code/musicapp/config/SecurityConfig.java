    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/", "/auth/**", "/songs", "/songs/**").permitAll() // trang chu, login, register, xem/nghe nhac public
                // Khu vuc rieng cho ADMIN
                .requestMatchers("/admin/**").hasRole("ADMIN")
                // Khu vuc can dang nhap (USER hoac ADMIN deu duoc, vd playlist)
                .requestMatchers("/playlists/**", "/profile/**").authenticated()
                // Con lai mac dinh cho phep, cac module khac (Song CRUD cua nguoi 2, Playlist cua nguoi 3) se tu sua lai matcher rieng
